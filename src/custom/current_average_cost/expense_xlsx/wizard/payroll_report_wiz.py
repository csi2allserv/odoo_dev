...
class payrollreportexcelwiz(models.TransientModel):
    ...
    def get_rules(self):
        ...
        heads = self.env['hr.expense.sheet'].search([('active', 'in', (True, False))], order='sequence asc')
        list = []
        for head in heads:
    ...
    def get_item_data(self):
        ...
        if self.from_date and self.date_end:
            ...
            payslip_ids=self.env['hr.expense.sheet'].search([('date_from','=',self.from_date),('date_to','=',self.date_end),('state','=','done')])
            if payslip_ids:
