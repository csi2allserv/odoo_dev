...
class PickingType(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee',
        ...
        oldname='technician_user_id', index=True
    )
    #Modificaciones
...
class Picking(models.Model):
    ...
    channels = fields.Selection([
        ...
        ('vehiculo', 'Vehiculo'),
        ('otro', 'Otro'),
        ],string='Canales', index=True, default='bodega')
    ...
    #           location_id = self.employee_id.property_stock_supplier.id
    #        else:
    #            customerloc, location_id = self.env['stock.warehouse']._get_partner_locations()
    ...
    #            location_dest_id = self.picking_type_id.default_location_dest_id.id
    #        elif self.employee_id:
    #            location_dest_id = self.employee_id.property_stock_customer.id
    #        else:
    #            location_dest_id, supplierloc = self.env['stock.warehouse']._get_partner_locations()#
    ...
    #            self.location_dest_id = location_dest_id
    #    # TDE CLEANME move into onchange_partner_id
    #    if self.employee_id and self.employee_id.picking_warn:
    #        if self.employee_id.picking_warn == 'no-message' and self.employee_id.parent_id:
    #            partner = self.employee_id.parent_id
    #        elif self.employee_id.picking_warn not in ('no-message', 'block') and self.employee_id.parent_id.picking_warn == 'block':
    #            partner = self.employee_id.parent_id
    #        else:
