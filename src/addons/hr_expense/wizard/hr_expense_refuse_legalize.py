...
class HrExpenseRefuseLegalizeWizard(models.TransientModel):
    ...
    _name = "hr.expense.refuse.legalize.wizard"
    ...
    def default_get(self, fields):
        res = super(HrExpenseRefuseLegalizeWizard, self).default_get(fields)
    ...
    @api.multi
    def expense_refuse_reason_legalize(self):
        self.ensure_one()
        if self.hr_expense_ids:
            self.hr_expense_ids.refuse_expense_legalize(self.reason)
        if self.hr_expense_sheet_id:
            self.hr_expense_sheet_id.refuse_sheet_legalize(self.reason)
